...
function(_qt5_Ftp_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5Ftp_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5Ftp_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5Ftp_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5Ftp_${Configuration}_${_lib}_PATH)
                    if(_Qt5Ftp_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5Ftp_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_Ftp_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_deps
        ${_Qt5Ftp_LIB_DEPENDENCIES}
    )
    set(_static_deps
        ${_Qt5Ftp_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set_property(TARGET Qt5::Ftp APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                 "${_deps}"
    )
    ...
    set(_static_link_flags "${_Qt5Ftp_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::Ftp APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::Ftp)
    ...
    set(_Qt5Ftp_OWN_INCLUDE_DIRS "${_qt5Ftp_install_prefix}/include/qt/" "${_qt5Ftp_install_prefix}/include/qt/QtFtp")
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_Ftp_process_prl_file(
            "${_qt5Ftp_install_prefix}/lib/libQt5Ftp.prl" RELEASE
            _Qt5Ftp_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5Ftp_STATIC_RELEASE_LINK_FLAGS
        )
    endif()
    ...
    set_property(TARGET Qt5::Ftp PROPERTY INTERFACE_QT_PLUGIN_TYPES "")
    ...
    if (_Qt5Ftp_PRIVATE_DIRS_EXIST)
        ...
        if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND NOT TARGET Qt::FtpPrivate)
            add_library(Qt::FtpPrivate INTERFACE IMPORTED)
            set_target_properties(Qt::FtpPrivate PROPERTIES
                INTERFACE_LINK_LIBRARIES "Qt5::FtpPrivate"
            )
        endif()
    endif()

    _populate_Ftp_target_properties(RELEASE "libQt5Ftp.a" "" FALSE)
    ...
    if (QT5_STRICT_PLUGIN_GLOB OR Qt5Ftp_STRICT_PLUGIN_GLOB)
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5Ftp_*Plugin.cmake")
    else()
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5Ftp_*.cmake")
    endif()
    ...
    macro(_populate_Ftp_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        set(imported_location "${_qt5Ftp_install_prefix}/lib/qt/plugins/${PLUGIN_LOCATION}")
        _qt5_Ftp_check_file_exists(${imported_location})
...
# Add a versionless target, for compatibility with Qt6.
if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND TARGET Qt5::Ftp AND NOT TARGET Qt::Ftp)
    add_library(Qt::Ftp INTERFACE IMPORTED)
    set_target_properties(Qt::Ftp PROPERTIES
        INTERFACE_LINK_LIBRARIES "Qt5::Ftp"
    )
endif()
